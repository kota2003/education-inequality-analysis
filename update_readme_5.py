...
Phase 07 update:
- Phase 07 row in the project status table flipped to complete.
- Findings section: "Available now" extended through Phase 07 with the
  synthesis layer (chained Cluster 1 finding, aggregate convergence
  figure, conditional policy framing); the standalone "Coming soon"
  subsection removed since the project is now closed at v1.0. Per the
  cumulative portfolio convention used in earlier projects (e.g.
  food-security-risk-analysis), six figures are embedded inline via
  raw GitHub URLs with italic captions, appended at the end of the
  Phase 04 / 05 / 06 / 07 entries: Phase 04 PCA scatter, Phase 05
  forest plot and per-cluster slopes, Phase 06 SHAP summary and
  per-cluster slopes, Phase 07 convergence. The embeds are visual
  punctuation only; no prose changes are made to the existing entries.
- Limitations and Future Work section replaced with its locked Phase 07
  final form per Phase 07 Step 01 Decision 5(b): one-paragraph
  epistemic stance + bulleted "Future Work" (synthetic control
  headline + IV / RD / DiD secondary) + bulleted "Structural
  Limitations" (4 items: MNAR, Gini measurement heterogeneity,
  country-level aggregation, temporal extrapolation cost).
